    ...
    <section class="py-4 sm:py-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            @foreach ($categories as $category)
                <a href="{{ route('books', ['selectedCategory' => $category]) }}" 
                   class="flex flex-col items-center p-4 bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow">
                    @switch($category)
                        ...
                        @case('hadis')
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 text-emerald-600 mb-2" viewBox="0 0 24 24" fill="none"
                    ...
                    @endswitch
                    <span class="text-sm font-medium text-gray-700">{{ str_replace('-', ' ', ucwords($category)) }}</span>
                </a>
            @endforeach
        </div>
    </section>
